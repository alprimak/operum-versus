# Claude Code (Single Agent) — Initial Prompt

Paste this into a Claude Code terminal session when starting the benchmark run.
...
```
I have a full-stack TypeScript application called TaskFlow in this repository.
It's a project management / task tracker built with Express + SQLite on the
...
listed below. Work through them one at a time. For each challenge, create a
...
- Work through them sequentially (single agent)
...
Start with B2 (easiest bug) to get oriented, then proceed through the rest.
```
