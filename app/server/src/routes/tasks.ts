...
function getTaskWithMembership(taskId: string, userId: string) {
  return taskRepository.findTaskWithMembership(taskId, userId) as any;
}

function getMentionedUsersForProject(projectId: string, mentions: string[]): any[] {
  return taskRepository.listMentionedUsersForProject(projectId, mentions);
}
...
function createMentionNotifications(
  ...
  for (const user of mentionedUsers) {
    ...
    notificationRepository.createMentionNotification(uuidv4(), user.id, commentId);
  }
}
taskRouter.get('/search', (req: AuthRequest, res: Response) => {
  try {
    const parsed = searchTaskSchema.parse(req.query);
    const tasks = taskRepository.searchTasks({
      userId: req.userId!,
      ...parsed,
    });
...
taskRouter.get('/project/:projectId', (req: AuthRequest, res: Response) => {
  const member = taskRepository.isProjectMember(req.params.projectId, req.userId!);
...
taskRouter.post('/', (req: AuthRequest, res: Response) => {
  try {
    ...
    const member = taskRepository.isProjectMember(data.project_id, req.userId!);
...
taskRouter.put('/:id', (req: AuthRequest, res: Response) => {
  try {
    ...
    const task = taskRepository.findTaskById(req.params.id) as any;
    if (!task) {
    ...
    // Verify membership
    const member = taskRepository.isProjectMember(task.project_id, req.userId!);
    ...
    if (nextAssigneeId) {
      const assigneeMember = taskRepository.isProjectMember(nextProjectId, nextAssigneeId);
    ...
    const updatedTask = taskRepository.findTaskById(req.params.id);
...
taskRouter.delete('/:id', (req: AuthRequest, res: Response) => {
  ...
  taskRepository.softDeleteTask(req.params.id);
...
taskRouter.get('/:taskId/comments', (req: AuthRequest, res: Response) => {
  ...
  const comments = taskRepository.listCommentsForTask(req.params.taskId);
...
taskRouter.post('/:taskId/comments', (req: AuthRequest, res: Response) => {
  try {
    ...
    taskRepository.createComment(commentId, req.params.taskId, req.userId!, content);
    ...
    const comment = taskRepository.findCommentWithAuthor(commentId);
...
taskRouter.put('/:taskId/comments/:commentId', (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingComment = taskRepository.findCommentByIdForTask(
    ...
    taskRepository.updateComment(req.params.commentId, content);

    createMentionNotifications(task.project_id, req.params.commentId, content, req.userId!);

    const comment = taskRepository.findCommentWithAuthor(req.params.commentId);
...
taskRouter.delete('/:taskId/comments/:commentId', (req: AuthRequest, res: Response) => {
  ...
  taskRepository.deleteComment(req.params.commentId);
  res.status(204).send();
});
